...
#   .\install-v108.ps1 -BundleUrl "https://..."
...
param(
    ...
    [string]$ReleaseTag = "v1.0.8",
    [string]$AssetName = "ngx-agent-v1.0.8.zip",
    [string]$BundleUrl = "",
    [string]$GitHubToken = "",
    [int]$Port = 9400,
    [switch]$Upgrade
...
# Extract to temp then copy files into InstallPath (handles ngx-agent-v108/ prefix)
$extractRoot = Join-Path $env:TEMP "ngx-agent-extract-$AgentVersion"
if (Test-Path $extractRoot) { Remove-Item $extractRoot -Recurse -Force }
Expand-Archive -Path $zipPath -DestinationPath $extractRoot -Force
$payload = $extractRoot
$nested = Join-Path $extractRoot "ngx-agent-v108"
